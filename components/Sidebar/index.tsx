    ...
    <aside className="bg-bg-surface p-4 w-60 flex flex-col sticky top-0 left-0 h-screen shadow overflow-y-auto scrollbar-thin">
      <div className="flex flex-col gap-12">
        ...
        <div className="flex items-center py-3">
          ...
          <div>
            <h1 className="font-bold text-2xl/tight">
              Toko<span className="text-brand-primary">Kita</span>
            </h1>
            <p className="text-[#4e4e4e] text-base/tight">Manajemen Toko</p>
          </div>
        </div>
        {/* Nav List */}
        <nav className="flex flex-col gap-4">
          {itemsMenu.map((item, i) => {
            ...
            return (
              <Link
                key={i}
                href={item.href}
                className={`flex items-center px-4 py-3 rounded-xl gap-6 transition-all ${isActive ? "bg-brand-light text-brand-primary" : "hover:bg-brand-light hover:text-brand-primary"}`}
              >
                <span>
          ...
          {/* Bagian bawah */}
        <div className="flex gap-2 px-4 py-6 bg-brand-light rounded-xl items-center">
          <div>
            <FontAwesomeIcon icon={faShop} className="text-brand-primary text-3xl"/>
          </div>
          <div >
